...
//   return <a href="#home" aria-label="Algeniux home" className="flex shrink-0 items-center"><img src={logoAsset} alt="Algeniux circuit brain logo" className={compact ? "h-24 w-auto" : "h-40 w-auto"} /></a>;
// }
...
/* ═══════════════════════════════════════════════════════════════
   NEW: TechHeroVisual — scroll-driven developer + brain animation
   ═══════════════════════════════════════════════════════════════ */
function TechHeroVisual() {
  ...
  // Layer speeds for parallax (different elements move at different rates)
  const bgY = useSpring(useTransform(scrollYProgress, [0, 1], [0, reduce ? 0 : -40]), springConfig);
  const brainY = useSpring(useTransform(scrollYProgress, [0, 1], [0, reduce ? 0 : -90]), springConfig);
  const brainRotate = useSpring(useTransform(scrollYProgress, [0, 1], [0, reduce ? 0 : 12]), springConfig);
  const brainScale = useSpring(useTransform(scrollYProgress, [0, 0.5, 1], [1, 1.08, 1]), springConfig);
  const personY = useSpring(useTransform(scrollYProgress, [0, 1], [0, reduce ? 0 : -50]), springConfig);
  const glowOpacity = useSpring(useTransform(scrollYProgress, [0, 0.5, 1], [0.4, 0.9, 0.4]), springConfig);
  const orbitRotate = useSpring(useTransform(scrollYProgress, [0, 1], [0, reduce ? 0 : 180]), springConfig);
  const codeLeftY = useSpring(useTransform(scrollYProgress, [0, 1], [0, reduce ? 0 : -130]), springConfig);
  const codeRightY = useSpring(useTransform(scrollYProgress, [0, 1], [0, reduce ? 0 : -80]), springConfig);

  return (
    <div ref={section} className="relative mx-auto aspect-square w-full max-w-[560px]">
        ...
        <svg viewBox="0 0 400 400" className="h-full w-full">
          <defs>
            <pattern id="circuit" x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse">
              <circle cx="20" cy="20" r="1.5" fill="var(--brand-cyan)" opacity="0.5" />
              <path d="M20 20 L20 8 M20 20 L32 20" stroke="var(--brand-blue)" strokeWidth="0.5" opacity="0.3" fill="none" />
            </pattern>
          </defs>
      ...
      <motion.div
        ...
        className="absolute inset-[5%] rounded-full border border-dashed border-brand-violet/20"
      />

      {/* Middle rotating orbit ring (counter) */}
      <motion.div
        animate={reduce ? false : { rotate: -360 }}
        transition={{ duration: 40, repeat: Infinity, ease: "linear" }}
        className="absolute inset-[15%] rounded-full border border-dashed border-brand-cyan/25"
      />
      ...
      <motion.div
        ...
        className="absolute inset-[25%] rounded-full border border-dashed border-brand-blue/20"
      />
      ...
      >
        <div className="h-full w-full rounded-full bg-gradient-to-br from-brand-violet/40 via-brand-blue/30 to-brand-cyan/40" />
      </motion.div>

      {/* THE BRAIN — circuit brain logo repurposed as "exposed naked brain" */}
      <motion.div
        style={{ y: brainY, rotate: brainRotate, scale: brainScale }}
        className="absolute left-1/2 top-[8%] w-40 -translate-x-1/2 sm:w-48"
      >
        <div className="relative">
          {/* Brain glow pulse */}
          <motion.div
            animate={reduce ? false : { scale: [1, 1.15, 1], opacity: [0.3, 0.6, 0.3] }}
            transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
            className="absolute inset-0 rounded-full bg-brand-cyan/20 blur-2xl"
          />
          <img
            src={logoAsset}
            alt="Neural circuit brain"
            className="relative w-full drop-shadow-[0_0_20px_rgba(0,0,0,0.3)]"
            style={{
              filter: "drop-shadow(0 0 8px var(--brand-cyan)) drop-shadow(0 0 16px var(--brand-violet))",
            }}
          />
          {/* Neural connection lines from brain down to person */}
          <svg
            viewBox="0 0 100 120"
            className="pointer-events-none absolute -bottom-28 left-1/2 w-20 -translate-x-1/2"
            style={{ overflow: "visible" }}
          >
            <motion.path
              d="M50 0 Q30 40 35 80 Q40 100 50 110"
              stroke="var(--brand-cyan)"
              strokeWidth="1.5"
              fill="none"
              strokeDasharray="4 4"
              animate={reduce ? false : { strokeDashoffset: [0, -20] }}
              transition={{ duration: 1.5, repeat: Infinity, ease: "linear" }}
              opacity="0.7"
            />
            <motion.path
              d="M50 0 Q70 40 65 80 Q60 100 50 110"
      ...
      {/* THE PERSON + PC — stylized SVG silhouette */}
      <motion.div
        style={{ y: personY }}
        className="absolute bottom-0 left-1/2 w-64 -translate-x-1/2 sm:w-72"
      >
        <svg
          viewBox="0 0 280 240"
          fill="none"
        ...
        >
          {/* Desk surface */}
          <rect x="20" y="180" width="240" height="8" rx="2" fill="var(--brand-blue)" opacity="0.4" />
          <rect x="30" y="188" width="220" height="4" rx="1" fill="var(--brand-violet)" opacity="0.3" />

          {/* Monitor */}
          <rect x="85" y="95" width="110" height="75" rx="4" fill="var(--surface)" stroke="var(--brand-blue)" strokeWidth="1.5" opacity="0.9" />
          <rect x="90" y="100" width="100" height="60" rx="2" fill="var(--background)" opacity="0.6" />
          {/* Code lines on screen */}
          <rect x="96" y="108" width="40" height="3" rx="1" fill="var(--brand-cyan)" opacity="0.8" />
          <rect x="96" y="118" width="60" height="3" rx="1" fill="var(--brand-violet)" opacity="0.7" />
          <rect x="96" y="128" width="50" height="3" rx="1" fill="var(--brand-blue)" opacity="0.7" />
          <rect x="96" y="138" width="35" height="3" rx="1" fill="var(--brand-cyan)" opacity="0.6" />
          {/* Stand */}
          <rect x="130" y="170" width="20" height="10" rx="1" fill="var(--brand-blue)" opacity="0.5" />
          <rect x="120" y="178" width="40" height="4" rx="1" fill="var(--brand-violet)" opacity="0.4" />

          {/* Person body silhouette */}
          <path
            d="M140 185 C140 185 125 185 115 175 C110 170 108 160 110 150 C112 140 120 135 130 132 L130 125 C130 125 128 115 135 110 C142 105 152 105 158 110 C165 115 163 125 163 125 L163 132 C173 135 181 140 183 150 C185 160 183 170 178 175 C168 185 153 185 153 185"
            fill="var(--surface)"
            stroke="var(--brand-violet)"
            strokeWidth="1.2"
            opacity="0.9"
          ...
          {/* Head / Brain area (open top — "naked brain") */}
          <ellipse cx="146.5" cy="108" rx="18" ry="22" fill="var(--surface)" stroke="var(--brand-blue)" strokeWidth="1.2" opacity="0.9" />
          {/* Open top of head showing brain interior */}
          <ellipse cx="146.5" cy="102" rx="14" ry="8" fill="var(--background)" stroke="var(--brand-cyan)" strokeWidth="1" opacity="0.8" />
          {/* Brain interior circuit pattern */}
          <circle cx="142" cy="100" r="2" fill="var(--brand-violet)" opacity="0.9" />
          <circle cx="152" cy="102" r="1.5" fill="var(--brand-cyan)" opacity="0.9" />
          <path d="M142 100 L146 98 L152 102" stroke="var(--brand-blue)" strokeWidth="0.8" fill="none" opacity="0.8" />

          {/* Arms typing */}
          <path
            d="M125 155 Q115 165 120 175 Q125 180 135 178"
          ...
          />
          <path
            d="M168 155 Q178 165 173 175 Q168 180 158 178"
            stroke="var(--brand-blue)"
            strokeWidth="3.5"
            strokeLinecap="round"
            fill="none"
          ...
          {/* Hands on keyboard */}
          <circle cx="132" cy="178" r="4" fill="var(--brand-cyan)" opacity="0.9" />
          <circle cx="161" cy="178" r="4" fill="var(--brand-cyan)" opacity="0.9" />

          {/* Keyboard */}
          <rect x="115" y="182" width="62" height="5" rx="1" fill="var(--brand-violet)" opacity="0.5" />
          <rect x="118" y="183" width="8" height="3" rx="0.5" fill="var(--brand-cyan)" opacity="0.6" />
          <rect x="128" y="183" width="8" height="3" rx="0.5" fill="var(--brand-cyan)" opacity="0.6" />
          <rect x="138" y="183" width="8" height="3" rx="0.5" fill="var(--brand-cyan)" opacity="0.6" />
          <rect x="148" y="183" width="8" height="3" rx="0.5" fill="var(--brand-cyan)" opacity="0.6" />
          <rect x="158" y="183" width="8" height="3" rx="0.5" fill="var(--brand-cyan)" opacity="0.6" />
          <rect x="168" y="183" width="6" height="3" rx="0.5" fill="var(--brand-cyan)" opacity="0.6" />

          {/* Floating particles / data points */}
          <motion.circle
            cx="60" cy="140" r="2.5" fill="var(--brand-cyan)"
            animate={reduce ? false : { y: [0, -8, 0], opacity: [0.3, 0.9, 0.3] }}
            transition={{ duration: 2.5, repeat: Infinity, ease: "easeInOut" }}
          />
          <motion.circle
            cx="220" cy="120" r="2" fill="var(--brand-violet)"
            animate={reduce ? false : { y: [0, -12, 0], opacity: [0.3, 0.8, 0.3] }}
            transition={{ duration: 3, repeat: Infinity, ease: "easeInOut", delay: 0.5 }}
          />
          <motion.circle
            cx="240" cy="160" r="1.5" fill="var(--brand-blue)"
            animate={reduce ? false : { y: [0, -6, 0], opacity: [0.3, 0.7, 0.3] }}
            transition={{ duration: 2, repeat: Infinity, ease: "easeInOut", delay: 1 }}
          />
        </svg>
      </motion.div>

      {/* Foreground floating code brackets */}
      <motion.div
        style={{ y: codeLeftY }}
        className="absolute right-[10%] top-[20%] text-brand-cyan/40"
      >
        <Braces className="size-8 sm:size-10" strokeWidth={1.5} />
      </motion.div>
      <motion.div
        style={{ y: codeRightY }}
        className="absolute left-[12%] top-[35%] text-brand-violet/40"
      >
        <Code2 className="size-6 sm:size-8" strokeWidth={1.5} />
      </motion.div>
    </div>
  );
}
...
/* ═══════════════════════════════════════════════════════════════
   UPDATED: Hero — uses TechHeroVisual instead of old logo visual
   ═══════════════════════════════════════════════════════════════ */
function Hero() {
  const section = useRef<HTMLElement>(null);
  const reduce = useReducedMotion();
  ...
  return (
    <section ref={section} id="home" className="relative flex min-h-[92vh] items-center overflow-hidden pt-28">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_72%_42%,color-mix(in_oklab,var(--brand-blue)_16%,transparent),transparent_35%)]" />
      <div className="relative mx-auto grid w-full max-w-7xl items-center gap-10 px-5 pb-16 lg:grid-cols-[1.12fr_.88fr] lg:px-8">

        {/* Left: copy */}
        <motion.div style={{ y: copyY, opacity: fade }} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .8, ease: [0.16, 1, 0.3, 1] }}>
          <div className="mb-7 inline-flex items-center gap-2 rounded-full border border-border bg-surface/70 px-4 py-2 text-sm text-muted-foreground">
          ...
          <h1 className="max-w-4xl text-4xl font-semibold leading-[1.06] text-foreground sm:text-6xl lg:text-7xl">
            Building Digital Experiences That <span className="brand-text">Think, Perform & Inspire.</span>
          </h1>
          <p className="mt-7 max-w-2xl text-base leading-8 text-muted-foreground sm:text-lg">
            I'm a software engineer focused on building modern websites, web applications, and digital experiences that combine clean engineering with exceptional design.
          </p>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            <Button asChild size="lg" className="h-12 brand-gradient px-6">
              <a href="#projects">View My Work <ArrowRight /></a>
            </Button>
            <Button asChild size="lg" variant="outline" className="h-12 border-border bg-surface/50 px-6">
              <a href="#contact">Let's Work Together</a>
            </Button>
          </div>
        </motion.div>

        {/* Right: NEW TechHeroVisual */}
        <TechHeroVisual />
